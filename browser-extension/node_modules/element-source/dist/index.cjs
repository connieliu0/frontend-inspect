...
var extractServerFramesFromDebugStack = (rootFiber) => {
  ...
  (0, import_bippy.traverseFiber)(
    rootFiber,
    (currentFiber) => {
      if (!(0, import_source.hasDebugStack)(currentFiber)) return false;
      const ownerStack = (0, import_source.formatOwnerStack)(currentFiber._debugStack.stack);
      if (!ownerStack) return false;
      for (const frame of (0, import_source.parseStack)(ownerStack)) {
...
var findNearestFiberNode = (node) => {
  if (!(0, import_bippy.isInstrumentationActive)()) return node;
  if ((0, import_bippy.getFiberFromHostInstance)(node)) return node;
  if (isElement(node)) {
    let current = node.parentElement;
    while (current) {
      if ((0, import_bippy.getFiberFromHostInstance)(current)) return current;
...
var fetchStackForNode = async (node) => {
  try {
    const fiber = (0, import_bippy.getFiberFromHostInstance)(node);
    if (!fiber) return null;
    const frames = await (0, import_source.getOwnerStack)(fiber);
...
var getReactStack = (node) => {
  if (!(0, import_bippy.isInstrumentationActive)()) return Promise.resolve([]);
...
var resolveSourceFromStack = (stack) => {
  ...
  for (const frame of stack) {
    if (frame.fileName && (0, import_source.isSourceFile)(frame.fileName)) {
      return {
        filePath: (0, import_source.normalizeFileName)(frame.fileName),
...
var resolveComponentName = async (node) => {
  ...
  const resolved = findNearestFiberNode(node);
  const fiber = (0, import_bippy.getFiberFromHostInstance)(resolved);
  if (!fiber) return null;
  let current = fiber.return;
  while (current) {
    if ((0, import_bippy.isCompositeFiber)(current)) {
      const name = (0, import_bippy.getDisplayName)(current.type);
